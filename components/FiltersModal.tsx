...
export const FiltersModal = ({ isOpen, onClose }: FiltersModalProps) => {
  ...
  const [propertyType, setPropertyType] = useState(searchParams.get("propertyType") || t('filters.any_type'));
  ...
  const handleApply = () => {
    ...
    if (propertyType !== t('filters.any_type')) params.set("propertyType", propertyType); else params.delete("propertyType");
  ...
  const handleClear = () => {
    ...
    setMinPrice("");
    setMaxPrice("");
    setPropertyType(t('filters.any_type'));
    setBeds(0);
    setBaths(0);
    setAmenities([]);
    router.push("/");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <main className="relative z-50 w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto hide-scroll p-8 space-y-10">
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              ...
              <span className="text-sm font-medium text-mosque">
                {minPrice || "0"} – {maxPrice || t('filters.any')}
              </span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option>{t('property_types.house')}</option>
                  <option>{t('property_types.apartment')}</option>
                  <option>{t('property_types.villa')}</option>
                  <option>{t('property_types.penthouse')}</option>
                </select>
          ...
          {/* Section 4: Amenities */}
          <section>
            <label className="block text-xs font-semibold text-nordic-muted uppercase tracking-wider mb-4">{t('filters.amenities')}</label>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                { id: "pool", label: t('filters.pool'), icon: "pool" },
                { id: "gym", label: t('filters.gym'), icon: "fitness_center" },
                { id: "parking", label: t('filters.parking'), icon: "local_parking" },
                { id: "ac", label: t('filters.ac'), icon: "ac_unit" },
                { id: "wifi", label: t('filters.wifi'), icon: "wifi" },
                { id: "patio", label: t('filters.patio'), icon: "deck" },
              ].map((item) => (
